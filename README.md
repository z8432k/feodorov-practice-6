...
8. **ВАЖНО!** Необходимо указать для какой платформы у вас собирается клиент, а для какой сервер! Для этого обратите внимание на комментарии в файле `CMakeLists.txt` **в корне проекта**. 
